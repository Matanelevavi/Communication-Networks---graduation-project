    ...
    def start(self):
        ...
        try:
            while True:
                ...
                if msg==DISCOVER_MSG:
                    if not self.pool_ip:
                        logging.warning(f"IP pool finished, Rejecting {cl_add}")
                        self.sock.sendto("DHCP: No IPs available".encode(ENCODING),cl_add)
                        continue
                    ...
                    if self.used_ips.get(cl_add)==wanted_ip:
                        ...
                        logging.warning(f"Invalid IP request from {cl_add}: {wanted_ip}")
